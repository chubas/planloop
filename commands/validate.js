...
function execute(options) {
  ...
  if (options.files) {
    // Use specified files
    filesToValidate = options.files.split(',').filter(file => fs.existsSync(file));
  } else {
    // Get changed files from git
    filesToValidate = getChangedFiles().filter(file => {
  ...
  if (filesToValidate.length === 0) {
    console.log('No relevant files to validate.');
    return 0;
  }
  
  console.log('🔍 Checking spec alignment for files...');
  if (options.verbose) {
